...
const Cart = () => {
  ...
  return (
    <motion.div
      {...slideIn}
      className="fixed z-50 top-0 right-0 w-300  md:w-[455px] bg-lightOverlay
     backdrop-blur-md shadow-md h-screen"
    >
      <div className="w-full flex items-center justify-between py-4 pb-12 px-8 ">
        <motion.i
          {...buttonClick}
      ...
      >
        {cart && cart?.length > 0 ? (
          <>
            ...
            <div
              className="bg-zinc-800 rounded-full w-full h-[20%] flex 
                   flex-col items-center justify-center px-4 py-6 gap-4 ">
            ...
            <motion.button 
            {...buttonClick}
            className="bg-orange-400 w-[70%] px-4 py-3 text-xl text-headingColor font-semibold
              hover:bg-orange-500 drop-shadow-md rounded-2xl" onClick={handelCheckOut}>check Out</motion.button>
