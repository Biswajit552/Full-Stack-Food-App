...
const SliderCard = ({data, index}) => {
  ...
  return ( 
     <div className="bg-lightOverlay hover:drop-shadow-lg backdrop-blur-md rounded-xl flex items-center
     justify-between relative px-4 py-2 w-full md:w-340 md:min-w-350 gap-3">
        <img src={data.imageURL} className="w-40 h-40 object-contain" alt=''/>
            <div className="relative pt-12">
                ...
                <HiCurrencyRupee className="text-red-600" />  {parseFloat(data.product_price).toFixed(2)}
                {/* ₹{data.product_price} */}
                </p>
                <motion.div
                {...buttonClick}
